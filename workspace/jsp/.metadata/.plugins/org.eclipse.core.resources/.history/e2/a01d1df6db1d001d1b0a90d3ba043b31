...
public class HaksaMemberDAO {
	...
	public HaksaMemberDTO getSelectOne(HaksaMemberDTO paramDto) {
		...
		conn = DB.dbConn();
		try {
			String sql = "select * from " + table_1 + " where hakbun = ?";
			pstmt = conn.prepareStatement(sql);
	...
	public int setInsert(HaksaMemberDTO paramDto) {
		...
		conn = DB.dbConn();
		try {
			String sql = "insert into " + table_1 + " values(seq_haksaMember.nextval, ?, ?, ?, ?, ?, ?, ?, sysdate)";
			pstmt = conn.prepareStatement(sql);
	...
	public int setUpdate(HaksaMemberDTO paramDto) {
		...
		conn = DB.dbConn();
		try {
			String sql = "update " + table_1 + " set phone = ?, parentPhone = ?, addr1 = ?, addr2 = ?, addr3 = ?, addr4 = ? where hakbun = ?";
			pstmt = conn.prepareStatement(sql);
	...
	public int setDelete(HaksaMemberDTO paramDto) {
		...
		conn = DB.dbConn();
		try {
			String sql = "delete from " + table_1 + " where hakbun = ?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, paramDto.getHakbun());
			result = pstmt.executeUpdate();
		} catch(Exception e) {
